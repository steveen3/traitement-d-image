...
Image subtractImages(const Image *image, const Image *image1) {
    if (image->width != image1->width || image->height != image1->height) {
        fprintf(stderr, "Les dimensions des images doivent être identiques pour la soustraction.\n");
        exit(EXIT_FAILURE);
    }
    ...
    for (int i = 0; i < result.height; ++i) {
        for (int j = 0; j < result.width; ++j) {
            ...
            int diff = image->pixels[index] - image1->pixels[index];
            result.pixels[index] = (diff < 0) ? 0 : diff;
        }
    }
